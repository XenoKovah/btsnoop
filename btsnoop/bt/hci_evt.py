...
def parse(data):
    ...
    ** [vol 2] Part E (Section 7.7.65) - Le Meta Event
    ...
    """
    hdr = EVT_HEADER()
    hdr.asbyte = struct.unpack("<BB", data[:2])[0]
...
def parse_evt_data(hci_evt_evtcode, hci_evt_subevtcode, data):
    ...
    hci_evt_info    = f'0x{hci_evt_evtcode:02x}, {evt_to_str(hci_evt_evtcode)}' if hci_evt_evtcode else '' # Only report evt info if available...
    hci_le_evt_info = f' / 0x{hci_evt_subevtcode:02x}, {subevt_to_str(hci_evt_subevtcode)}' if hci_evt_subevtcode else '' # Only report subevt info if available...
    ...
    if hci_evt_evtcode == INV_HCI_EVENTS_LOOKUP["EVENT LE_Meta_Event"]:
        ...
        if hci_evt_subevtcode == INV_HCI_LE_META_EVENTS_LOOKUP["EVENT LE_Connection_Complete"]:
            assert(len(data) == 18)
            bdaddr = pkt_bytes_to_bdaddr(data[5:11])
    ...
    elif hci_evt_evtcode == INV_HCI_EVENTS_LOOKUP["EVENT Connection_Complete"]:  # -> status (1), connection handle (2), BD_ADDR (6), link type (1), encrytion enabled (1)
        assert(len(data) == 11)

    elif hci_evt_evtcode == INV_HCI_EVENTS_LOOKUP["EVENT Connection_Request"]:  # -> Connection_Request -> BD_ADDR (6), CoD (3), link type (1)
        assert(len(data) == 10)
        bdaddr = pkt_bytes_to_bdaddr(data[:6])
        cod = pkt_bytes_to_cod(data[6:9])
        lt = struct.unpack("<B", data[9:])[0]

        print(f'{prefix}:: bdaddr=[{bdaddr}] / cod=[{cod}] / lt=[{lt}]')

        return (bdaddr, cod, lt)

    elif hci_evt_evtcode == INV_HCI_EVENTS_LOOKUP["EVENT Command_Complete"]:  #
        response_opcode = struct.unpack("<BB", data[1:3])
        response_opcode_str = " ".join("{:02X}".format(b) for b in response_opcode)
        # print(f"{prefix}:: response_opcode_str = [{response_opcode_str}] <<< responding to command with HEX opcode")
        return response_opcode_str
